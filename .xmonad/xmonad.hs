...
myWorkspaces :: [String]
myWorkspaces = clickable . (map xmobarEscape) $ [">_","{}","www","md",">>","gg",".."]
    where
...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    -- launch dmenu
    , ((modm,               xK_p     ), spawn "dmenu_run -nb black -nf grey -p \"RUN:\"")

	-- launch slock
    , ((modm,               xK_Next  ), spawn "slock")
    ...
    -- Cycle through all workspace
    , ((modm,               xK_Tab),    nextWS)
    , ((modm .|. shiftMask, xK_Tab),    prevWS)

    -- Cycle through empty workspace 
    , ((modm     , xK_n), moveTo Next EmptyWS)
    , ((modm     , xK_b), moveTo Prev EmptyWS)
...
myManageHook = composeAll
	...
	, className =? "obsidian"		--> viewShift "md" ]
...
myStartupHook = do
    ...
    spawnOnce "feh --bg-fill path/l --bg-fill path/r ~/Pictures/Wallpapers/Rei.png"
    spawnOnce "setxkbmap -option caps:swapescape"
    spawnOnce "nm-applet"
    spawnOnce "trayer --edge top --alight right --widthtype request --padding 6 --SetDockType true --SetPartialStrut true --expand true --monitor 1 --height 20"
   ...
   xmonad $ docks $ def {
      ...
      -- hooks, layouts
        ...
        logHook            = dynamicLogWithPP xmobarPP
                     ...
                     , ppVisible = xmobarColor "#458588" ""
                     , ppHidden = xmobarColor "#458588" ""
                     , ppHiddenNoWindows = xmobarColor "#3c3836" ""
